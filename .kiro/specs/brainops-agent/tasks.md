...
- [ ] 1.1 Initialize `brainops/` TypeScript project with package.json, tsconfig.json, and dependencies (langgraph, @langchain/langgraph-checkpoint-postgres, pg, nats, @aws-sdk/client-bedrock-runtime, @aws-sdk/client-secrets-manager)
- [ ] 1.2 Create `src/config/index.ts` with BrainOpsConfig interface and environment variable loading
- [ ] 1.3 Create Dockerfile (multi-stage build, Node.js 20 Alpine)
- [ ] 1.4 Create `src/health/endpoints.ts` with /healthz and /readyz HTTP endpoints
- [ ] 1.5 Create `src/index.ts` entry point that wires startup: load config → connect DB → connect NATS → start agent → start health server
